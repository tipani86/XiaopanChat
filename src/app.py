...
# Render footer
with footer:
    st.info("暂停GPT-4对外公开服务的通知：亲爱的小潘AI用户，您好！因GPT-4模型成本实在过高，打赏金额远远支撑不了API接口调用的费用，我们不得不暂停GPT-4模型无条件对外公开的服务。同时，我们依然欢迎您使用基于GPT-3.5-Turbo（ChatGPT大众版）模型的服务（链接如下）：", icon="ℹ️")
    st.markdown("<a href='https://chat.xiaopan.ai' target='_blank'>https://chat.xiaopan.ai</a>", unsafe_allow_html=True)
